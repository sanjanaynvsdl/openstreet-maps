...
export default function MapScreen() {
  ...
  // Function to check for WebView load status
  const handleWebViewLoad = () => {
    console.log('[PERF] WebView loaded at:', new Date().toISOString());
    setIsWebViewReady(true);
  };

  // Listen for WebView messages including performance metrics
  const onWebViewMessage = (event: any) => {
    try {
      const data = JSON.parse(event.nativeEvent.data);
      console.log('[PERF] WebView message received:', data.type);
      
      if (data.type === 'ready' || data.type === 'mapInitialized') {
        setIsWebViewReady(true);
      } else if (data.type === 'routeRendered') {
        console.log(`[PERF] Route rendering completed in WebView at: ${new Date(data.timestamp).toISOString()}`);
      }
    } catch (error) {
    ...
    <View style={styles.container}>
      <WebView
        ...
        source={{ 
          ...
          uri: "https://sanjanaynvsdl.github.io/map/" 
          // For local testing, you can use:
          // uri: require('../assets/map.html') 
        }}
        originWhitelist={['*']}
        ...
        onError={(error) => console.error('[PERF] WebView error:', error)}
